...
public abstract class Entity {
    ...
    protected float FrameTimer { get; set; }
    ...
    public float LifeTime { get; set; } // How long entity has been alive
    ...
    public virtual void Render(DoubleBuffer buffer) {
        ...
        string[] frame = Frames[CurrentFrameIndex];
        string[]? colorMask = ColorMasks.Length > CurrentFrameIndex ? ColorMasks[CurrentFrameIndex] : null;

        var startX = (int)X;
        var startY = (int)Y;

        for (var lineIdx = 0; lineIdx < frame.Length; lineIdx++) {
    ...
    public bool CollidesWith(Entity other) {
        (int x1, int y1, int w1, int h1) = GetBounds();
        (int x2, int y2, int w2, int h2) = other.GetBounds();
